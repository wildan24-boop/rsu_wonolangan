...
public function ajax_list_menu()
{
    ...
    foreach ($list as $us) {
		...
		<a class="dropdown-item btn btn-sm btn-danger" href="javascript:void(0)" title="Hapus" onclick="delete_menu('."'".$us->id."'".')"><i class="fa fa-trash-alt ml-4"></i> Delete</a>';
        $row[] = ($us->menu);
        $data[] = $row;
    }
...
public function ajax_list_submenu()
{
    ...
    foreach ($list as $us) {
		...
		<a class="dropdown-item btn btn-sm btn-danger" href="javascript:void(0)" title="Hapus" onclick="delete_submenu('."'".$us->id."'".')"><i class="fa fa-trash-alt ml-4"></i> Delete</a>';
        $row[] = ($us->title);
        $row[] = ($us->menu);
        $row[] = ($us->url);
        $row[] = ($us->icon);
